<?php
/**
*
* pm_pgp[Russian]
*
*
*/

/**
* DO NOT CHANGE
*/
if (!defined('IN_PHPBB'))
{
   exit;
}

if (empty($lang) || !is_array($lang))
{
	$lang = array();
}

// DEVELOPERS PLEASE NOTE
//
// All language files should use UTF-8 as their encoding and the files must not contain a BOM.
//
// Placeholders can now contain order information, e.g. instead of
// 'Page %s of %s' you can (and should) write 'Page %1$s of %2$s', this allows
// translators to re-order the output of data while ensuring it remains correct
//
// You do not need this where single placeholders are used, e.g. 'Message %d' is fine
// equally where a string contains only two placeholders which are used to wrap text
// in a url you again do not need to specify an order e.g., 'Click %sHERE%s' is fine
//
// Some characters you may want to copy&paste:
// ’ » “ ” …
//

$lang = array_merge($lang, array(
    'PM_PGP'        => 'PGP for private messages',
    'INSTALL_PM_PGP'        => 'Install PGP for private messages',
    'INSTALL_PM_PGP_CONFIRM'  => 'Confirm installation of PGP for private messages',
    'UPDATE_PM_PGP'        => 'Update PGP for private messages',
    'UPDATE_PM_PGP_CONFIRM'  => 'Confirm updating of PGP for private messages',
    'UNINSTALL_PM_PGP'        => 'Uninstall PGP for private messages',
    'UNINSTALL_PM_PGP_CONFIRM' => 'Confirm uninstallation of PGP for private messages',
	'PGP_KEY'	=>	'Открытый ключ PGP',
	'REMOVE_PGP_KEY'	=>	'Удалить открытый ключ PGP из базы данных?',
	'PGP_KEY_NOT_UPLOADED'	=>	'Ключ PGP не загружен',
// Install block
	'PM_PGP_INSTALLED'			=> 'Blah-blah-blah',
	'PM_PGP_INSTALLED_EXPLAIN'  => '<strong>ВНИМАНИЕ!<br />Рекомендуется запускать данную установку только после выполнения инструкции по внесению изменений в код файлов конференции (или выполнения установки с помощью AutoMod)!<br />Также настоятельно рекомендуется включить опцию Отображать все результаты (ниже)!</strong>',
	'PM_PGP_RETURN_UPDATE'	=> 'Обновление данных в таблице _pgp_keys table',
	'PM_PGP_RETURN_REMOVE'	=> 'Проверка удаления модуля',
	'PM_PGP_RETURN_CACHE'	=> 'Проверка обновления кеша',
	'UCP_PROFILE_PGP_KEY'	=> 'Управление ключом PGP',
	'UCP_PROFILE_PGP_NOTICE'	=> 'Обратите внимание, что ...[информация о ключах PGP будет добавлена позже]',
	'YOUR_PUBLIC_PGP_KEY'	=> 'Твой открытый ключ PGP',
	'PGP_KEY_FINGERPRINT'	=> 'Отпечаток ключа',
    'PGP_KEY_DELETED'       => 'Твой открытый ключ PGP был удалён из базы данных форума.',
    'BAD_PGP_KEY'           => 'Операция прервана. Невозможно расчитать отпечаток твоего ключа PGP. Неверный ключ?',
    'PGP_KEY_UPLOADED'      => 'Твой открытый ключ PGP был загружен в базу данных форума.',
));
?>
